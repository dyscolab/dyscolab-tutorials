...
def _():
    ...
    class CellInteraction(System): # Models are a class thath inherits from System
        X: Variable = initial(default=10) # Variable X with default initial condition 10
        ...
        eq_x = X.derive() << X* (a*X*(1-X/c)-d-b*Y) # Equation dX/dt = X(aX(1-X/c)-d-bY)
...
def _():
    ...
    def get_refractory(ds): # Function to get refractory period
        ...
        times_below_threshold = data_after_max.time.where(data_after_max < 0.01 * max_val, drop=True)
...
def _(mo):
    mo.md(r"""
    And we can then create a `Sweeper` for it
    """)
    return
...
def _(CellInteraction, Sweeper, get_refractory, np, plt, sim):
    ...
    refractory_period = sweeper.sweep(sim = sim, save_at=np.linspace(0,50,1000), parameter=CellInteraction.g, values=np.linspace(0.1, 2, 50)) # Do sweep, we give a save_at for each simulation run and values to sweep the parameter
...
def _(mo):
    mo.md(r"""
    And we can calculate the sensitivity using a [savitzky-golay filter](https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.savgol_filter.html) to smooth it first
    """)
    return
...
def _(np, plt, refractory_period):
    ...
    log_r, log_t = np.log(refractory_period["result"].values), np.log(refractory_period["g"].values)
    sensitiviy = savgol_filter(log_r,5, 2, deriv=1, delta=log_t[1]-log_t[0]) 
    # sensitiviy = np.gradient(np.log(refractory_period["result"].values), np.log(refractory_period["g"].values))
...
def _(CellInteraction, Simulator, System, assign, np):
    ...
    class LinkedCellInteraction(System):
        a: Constant = assign(default=1, constant=True) # Constants to represnt initial conditions
        ...
        threshold_110: Constant = assign(default = 1.1* c/2 *(1-real.sqrt(1-(4*d)/(c*a))), constant=True)
...
def _(mo):
    mo.md(r"""
    And use another `Sweeper` to get the pulse amplitude on each run
    """)
    return
...
def _(LinkedCellInteraction, Sweeper, lsim, np):
    def get_pulse_amplitude(ds):
        ...
        verified = np.abs(last) < 0.9 * np.abs(max) # Check X has decayed 10% from max to know ew seimlated long enough
        return {"max" : max, "verified":verified} # When we return a dictionary Sweep
    amp_sweeper = Sweeper(func = get_pulse_amplitude)
    pulse_amplitude = amp_sweeper.sweep(sim = lsim, save_at=np.arange(0,3,0.001), parameter=(LinkedCellInteraction.a), values=np.linspace(0.5, 2.5, 50))
    pulse_amplitude
...
def _(np, plt, pulse_amplitude):
    ...
    print(f"All simulations decayed at least 10% from maximum: {np.all(pulse_amplitude["verified"]).values}")
    plt.show()
    return
...
def _(mo):
    mo.md(r"""
    And calculate and plot the sensitivity $\frac{d\log(A)}{d\log(a)}$
    """)
    return
...
@app.cell
def _(np, plt, pulse_amplitude):
    amp_sensitiviy = np.gradient(np.log(pulse_amplitude["max"].values), np.log(pulse_amplitude["a"].values)) 
...
def _(LinkedCellInteraction, amp_sensitiviy, amp_sweeper, lsim, np, plt):
    ranges = {LinkedCellInteraction.b: np.linspace(0.1, 5, 50), 
              LinkedCellInteraction.c: np.linspace(200, 500, 50),
              LinkedCellInteraction.d: np.linspace(0.2, 10, 50), # d range start changed from 0.01 to 0.1 since pulse wasn't seen
    ...
    for var, values in ranges.items():
        sweep_result = amp_sweeper.sweep(sim = lsim, save_at=np.arange(0,25,0.001), parameter=var, values= values)
        results[var] = np.gradient(np.log(sweep_result["max"].values), np.log(sweep_result[str(var)].values))
